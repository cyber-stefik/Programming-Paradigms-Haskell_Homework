...
outNeighbors node graph =
    S.fromList [snd x | x <- S.toList (edges graph), fst x == node]
...
inNeighbors node graph = 
    S.fromList [fst x | x <- S.toList (edges graph), snd x == node]
...
removeNode node graph = 
    (S.fromList [x | x <- S.toList (nodes graph), x /= node],
    S.fromList [x | x <- S.toList (edges graph), fst x /= node, snd x /= node])
...
splitNode old news graph = 
    let newNodes = S.toList (fst (removeNode old graph))
        newEdges = S.toList (snd (removeNode old graph))
        inNodes = S.toList (inNeighbors old graph)
        outNodes = S.toList (outNeighbors old graph)
    in (S.fromList (newNodes ++ news),
        S.fromList ((L.union [(x, y) | x <- inNodes, y <- news] [(x, y) | x <- news, y <- outNodes]) ++ newEdges))
...
mergeNodes prop node graph = 
    let nodes = S.toList (fst graph)
        filteredNodes = (filterNot prop nodes)
        myEdges = S.toList (edges graph)
    in case ((filterNot prop nodes) == nodes) of
        True -> graph
        _ -> (S.fromList (filteredNodes ++ [node]),
                S.fromList (map (\x -> if
                    | prop (fst x) && prop (snd x) -> (node, node)
                    | prop (fst x) -> (node, snd x)
                    | prop (snd x) -> (fst x, node)
                    | otherwise -> x) myEdges))