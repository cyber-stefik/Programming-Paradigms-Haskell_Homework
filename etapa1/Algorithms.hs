...
searchHelper :: Ord a => Graph a -> [a] -> [a] -> ([a] -> [a] -> [a]) -> [a]
searchHelper graph queue visited f 
    | null queue = visited
    | notElem (head queue) visited = searchHelper graph (f (tail queue) (S.toList (outNeighbors (head queue) graph))) (visited ++ [head queue]) f
    | otherwise = searchHelper graph (tail queue) visited f
...
bfs = \node graph -> search (\x y -> x ++ y) node graph
...
dfs = \node graph -> search (\x y -> y ++ x) node graph
...
countIntermediate from to graph
    | notElem from (bfs from graph) || notElem to (bfs from graph) || notElem from (dfs from graph) || notElem to (dfs from graph) = Nothing
    | otherwise = Just ((head (L.elemIndices to (bfs from graph))) - (head (L.elemIndices from (bfs from graph))) - 1,
                        (head (L.elemIndices to (dfs from graph))) - (head (L.elemIndices from (dfs from graph))) - 1)
