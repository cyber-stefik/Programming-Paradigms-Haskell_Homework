...
{-
    data GenericGraph a = generic
    {
        node :: GenericGraph a
        nodes :: GenericGraph a -> S.set a
        adj :: GenericGraph a -> GenericGraph b -> S.set (GenericGraph a, GenericGraph b)
    }
...
splitNode node targets = 
    let graphFromList targets res
            | length targets == 0 = res
            | otherwise = (graphFromList (tail targets)) (Overlay (Node (head targets)) res)
    in extend (\x -> if x == node then (graphFromList targets Empty) else (Node x))
{-
    *** TODO ***
...
mergeNodes :: (a -> Bool)       -- proprietatea îndeplinită de nodurile îmbinate
           ...
           -> AlgebraicGraph a  -- graful existent
           -> AlgebraicGraph a  -- graful obținut
mergeNodes prop node = 
    fmap (\x -> if (prop x) then node else x)
...
filterGraph prop (Node a)
    | (prop a) = (Overlay (Node a) Empty)
    | otherwise = Empty
filterGraph prop (Overlay graph1 graph2) = (Overlay (filterGraph prop graph1) (filterGraph prop graph2))
filterGraph prop (Connect graph1 graph2) = (Connect (filterGraph prop graph1) (filterGraph prop graph2))
...
removeNode node graph = 
    filterGraph (\x -> if x == node then False else True) graph