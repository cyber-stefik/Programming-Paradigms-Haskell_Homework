...
mapSingle f xs = 
    (map (\x ->
        (fst (splitAt x xs)) ++ [(f (head (snd (splitAt x xs))))] ++ (snd (splitAt (x + 1) xs))) (positions xs))


    -- (map (\pos -> 
    --     let (x, y:z) = splitAt (head (L.elemIndices pos xs)) xs
    --     in x ++ ((f y) : z)) xs)
    
    -- Eq a
    -- | null xs = []
    -- | otherwise = foldl (\acc x -> acc ++ [(foldl (\acc2 x2 -> if x2 == x then acc2 ++ [(f x2)] else acc2 ++ [x2]) [] xs)]) [] xs
